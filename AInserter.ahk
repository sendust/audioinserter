...
title = SBS Audio Inserter by sendust 2019/9/18
ffmpeg := Object()
...
param_proxy := Object() 			 ; low res proxy parameter
...
Gui, add, Button, w70 h50 xp+530 yp gplay_media1, 재생`r`n(비디오)
Gui, add, Button, w60 h50 xp+80 yp hwndhlowres glowres , 저해상도
...
Gui, add, button, xp yp+70 w120 h50 gsel_target, 출력폴더
...
button_control := [hjob1, hjob2, hlowres]
...
;param_proxy["proxy_mp4_video"] := "-pix_fmt yuv420p -c:v libx264 -preset:v veryfast -g 30 -crf 24 -filter_complex ""[0:v]yadif=0:-1:0[vd];[vd]scale=1280:720"
param_proxy["proxy_mp4_video"] := "-pix_fmt yuv420p -c:v libx264 -preset:v veryfast -g 30 -crf 24 -filter_complex ""[0:v]field=type=top[vf];[vf]scale=960:540[vs];[vs]setdar=1.7778"
param_proxy["proxy_mp4_6 channels-1"] := ";[0:a]aresample=async=10000[are];[are]pan=stereo|c0=c0+c2|c1=c1+c3"" -c:a aac -b:a 128k -ac 2 -strict -2"
param_proxy["proxy_mp4_2 channels-1"] := ";[0:a]aresample=async=10000"" -c:a aac -b:a 128k -ac 2 -strict -2 "
param_proxy["proxy_mp4_2 channels-2"] := param_proxy["proxy_mp4_2 channels-1"]
param_proxy["proxy_mp4_2 channels-4"] := param_proxy["proxy_mp4_2 channels-1"]
param_proxy["proxy_mp4_mono-1"] := ";[0:a]aresample=async=10000[are];[are]pan=stereo|c0=c0|c1=c0"" -c:a aac -b:a 128k -ac 2 -strict -2  -y"
param_proxy["proxy_mp4_mono-2"] := ";[0:a]amerge=inputs=2[am];[am]aresample=async=10000[are];[are]pan=stereo|c0=c0|c1=c1"" -c:a aac -b:a 128k -ac 2 -strict -2 "
param_proxy["proxy_mp4_mono-4"] := ";[0:a]amerge=inputs=4[am];[am]aresample=async=10000[are];[are]pan=stereo|c0=c0+c2|c1=c1+c3"" -c:a aac -b:a 128k -ac 2 -strict -2 "
param_proxy["proxy_mp4_mono-6"] := param_proxy["proxy_mp4_mono-4"]
param_proxy["proxy_mp4_mono-8"] := param_proxy["proxy_mp4_mono-4"]
param_proxy["proxy_mp4_noaudio"] := """ -an "

gosub, audiomonsel		; get audio monitor selection

consoleout("`r`n출력 폴더는 " . ffmpeg.path_dst)
return
...
runstring := get_proxystring(ffmpeg, media1)
...
if InStr("mxf|mov|mp4", splitpath_extension(A_GuiEvent))
{
...
get_proxystring(ffmpeg, media1)
{
	global param_proxy, hstatus
  
	SplitPath, % media1.fullpath, outfilename, outdir, outextension, outnamenoext, outdrive

	outname_noext = %outnamenoext%_저해상도
	binary := ffmpeg.binary
	in1 := media1.fullpath
	
	string_filter := param_proxy["proxy_mp4_video"] . param_proxy["proxy_mp4_" . media1.audio_format]

	path_dst := ffmpeg.path_dst
	outfile = %path_dst%\%outname_noext%.mp4
	runstring = %binary% -i "%in1%" %string_filter%  -y -hide_banner  "%outfile%"
	consoleout(runstring)
	GuiControl,, %hstatus%, Output file is %outfile%
...
consoleout("출력 폴더는 " . ffmpeg.path_dst)
return
